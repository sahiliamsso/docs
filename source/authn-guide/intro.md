...
Implementation guides for a number of two-factor authentication mechanisms can be found in the pages that follow in this User Authentication Guide.
...
All authentications that happen at the Gluu Server are routed through oxAuth, 
the software that includes Gluu's OpenID Connect Provider (OP) implementation. 
Using the OpenID Connect `acr_value`, an application can request a certain type of 
